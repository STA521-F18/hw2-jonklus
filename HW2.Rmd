...
ggplot(UN3_clean,aes(x=log(Pop), y=log(ModernC)))+geom_point() + geom_smooth(method="lm")
```
...
kable(CI_w_coeff,'markdown',c(2,2,2),row.names = TRUE, col.names = c("Coeff","Lower","Upper"), align = "c", 
...
The final model is:
ModernC = 4.10 + (4.86)log(PPgdp) + (1.44)log(Pop) - (9.28)Fertility + 
(4.70)Change + (0.20)Frate
...
\text{we know that H is a projection matrix and is therefore idempotent, so I-H is also idempotent: } (I-H)(I-H)=(I-H)\\
