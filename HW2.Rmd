...
```{r}
y_frate = ggplot(data = UN3, aes(x = Frate, y = ModernC)) + geom_point(na.rm = TRUE) + labs(title = "Contraceptive Use vs. Adult Female Economic Activity", caption = "% females using modern contraception plotted against % females >15 years that are economically active by country in UN3 data")

y_purban = ggplot(data = UN3, aes(x = Purban, y = ModernC)) + geom_point(na.rm = TRUE) + labs(title = "Contraceptive Use vs. % Urban Pop", caption = "% females using modern contraception plotted against % of population that is urban by country in UN3 data")

y_pop = ggplot(data = UN3, aes(x = Pop, y = ModernC)) + geom_point(na.rm = TRUE) + labs(title = "Contraceptive Use vs. Population", caption = "% females using modern contraception plotted against total population by country in UN3 data")
...
There are some noteable potential outliers. In the chart A, 
## Model Fitting
...
```{r}
boxTidwell(ModernC ~ PPgdp + Pop + Fertility, ~Change + Purban, UN3_clean)
```
```{r}
boxTidwell(ModernC ~ PPgdp + Pop + Fertility, ~Change + Purban, UN3_clean)
```
